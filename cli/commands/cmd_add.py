...
def fake_users():
    ...
    while True:
        ...
        }#'coins': 100,'last_bet_on': last_bet_on,
...
def fake_files():
    ...
    while i<20:
        ...
        if i==2:
            ...
            if random_percent >= 20:
                random_trail1 = str(int(round((random.random() * 1000))))
                filename = fake.first_name() + random_trail1
            #last_bet_on = created_on
            else:
                random_trail2 = str(int(round((random.random() * 1000))))
                filename = fake.last_name() + random_trail2
            #last_bet_on = created_on
            #last_bet_on = None
    ...
    return _bulk_insert(File, data, 'files')


@click.command()
def real_files():
    """
    Add files from filesystem to DB.
    """
    i=0
    data = []
    filefromfolder = os.listdir('UWdataweb/dataweb/DataFiles')
...
def fake_all(ctx):
    ...
    :return: None
    """
    ctx.invoke(fake_users)
    ctx.invoke(fake_files)
    '''
    ctx.invoke(invoices)
    ctx.invoke(bets)
'''
...
cli.add_command(fake_users)
cli.add_command(fake_files)
'''
cli.add_command(invoices)
cli.add_command(bets)
'''
cli.add_command(fake_all)
